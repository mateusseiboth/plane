...
export const instanceModule = new Elysia({ prefix: "/instances" })
  ...
  .patch("/configurations/", async ({ body, headers, set }) => {
    ...
    if (!instance) {
      set.status = 400;
      return { detail: "Instância não encontrada." };
    }
    ...
    const merged: Record<string, string> = {
      ...existing,
      ...Object.fromEntries(Object.entries(incoming).map(([k, v]) => [k.toUpperCase(), v === null ? "" : String(v)])),
    };
  ...
  .get("/priority-sla/", async ({ headers, set }) => {
    ...
    return {
      priority_sla: cfg && typeof cfg === "object" ? { ...DEFAULT_PRIORITY_SLA, ...cfg } : DEFAULT_PRIORITY_SLA,
    };
  })
  ...
  .patch("/priority-sla/", async ({ body, headers, set }) => {
    ...
    const current =
      existing.priority_sla && typeof existing.priority_sla === "object" ? existing.priority_sla : DEFAULT_PRIORITY_SLA;
    ...
    await prisma.instance.update({
      where: { id: instance.id },
      data: { configurations: { ...existing, priority_sla: next } },
    });
  ...
  .get("/workspace-slug-check/", async ({ query, headers, set }) => {
    ...
    const taken =
      RESTRICTED.includes(slug.toLowerCase()) ||
