...
export function createCurriculoService({ dao, storage, now, newId }: CurriculoDeps) {
  ...
  /** Caminho único de gravação: valida, guarda o arquivo e grava a linha. */
  const save = async (
    workspaceId: string,
    { data, errors }: { data: CurriculoData; errors: FieldErrorItem[] },
    arquivo: Blob | null,
  ...
  const readConfig = async (workspaceId: string) => {
    ...
    return {
      retention_days: config?.retentionDays ?? RETENCAO_PADRAO_DIAS,
      site_enabled: config?.siteEnabled ?? false,
    };
  };
  ...
  return {
    /** Currículo que o robô do WhatsApp manda. */
    create(workspaceId: string, body: Record<string, unknown>, arquivo: Blob | null, nomeDoArquivo: string) {
      return save(workspaceId, validateCurriculoInput(body), arquivo, nomeDoArquivo);
    },

    /** Inscrição da página pública "Trabalhe conosco" (origem `site`). */
    createDoSite(workspaceId: string, body: Record<string, unknown>, arquivo: Blob | null, nomeDoArquivo: string) {
      return save(workspaceId, validateInscricaoDoSite(body, now()), arquivo, nomeDoArquivo);
    },
    ...
    /**
     * Prazo de guarda e o interruptor da página pública. Cada campo só é
     * tocado se vier no corpo: a tela salva um de cada vez.
     */
    async saveConfig(workspaceId: string, body: Record<string, unknown>) {
      const atual = await readConfig(workspaceId);
      const isPrazoRecebido = body.retention_days !== undefined;
      if (isPrazoRecebido) requireNoFieldErrors(validateRetencao(body.retention_days));
      const config = await dao.saveConfig(workspaceId, {
        retentionDays: isPrazoRecebido ? Number(body.retention_days) : atual.retention_days,
        siteEnabled: body.site_enabled === undefined ? atual.site_enabled : body.site_enabled === true,
      });
      return { retention_days: config.retentionDays, site_enabled: config.siteEnabled };
    },

    isSiteAberto: async (workspaceId: string) => (await dao.findConfig(workspaceId))?.siteEnabled === true,
