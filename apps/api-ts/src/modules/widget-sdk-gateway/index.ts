...
const widgetAuthPlugin = new Elysia({ name: "widget-auth" }).use(authPlugin).derive({ as: "global" }, async (ctx) => {
...
export const widgetSdkGatewayModule = new Elysia({ prefix: "/widget-sdk" })
  ...
  .get("/worker-items/:id", async ({ params: { id }, widget, set }) => {
    requirePermission(widget, "worker-items.read", set);
  ...
  .get("/worker-items/stats", async ({ query, widget, set }) => {
    requirePermission(widget, "worker-items.read", set);
    const q = query as any;
    const where: any = { deletedAt: null, isDraft: false };
    if (q.workspace_slug) {
  ...
  .get("/intakes", async ({ query, widget, set }) => {
  ...
  .get("/intakes/:id", async ({ params: { id }, widget, set }) => {
    requirePermission(widget, "intakes.read", set);
  ...
  .get("/intakes/stats", async ({ query, widget, set }) => {
    requirePermission(widget, "intakes.read", set);
    const q = query as any;
    const where: any = { deletedAt: null };
    if (q.workspace_slug) {
  ...
  .get("/actions", async ({ query, widget, set }) => {
  ...
  .get("/actions/:id", async ({ params: { id }, widget, set }) => {
    requirePermission(widget, "actions.read", set);
    const issue = await prisma.issue.findFirst({
      where: { id, deletedAt: null },
      include: { state: { select: { id: true, name: true, group: true } } },
  ...
  .get("/actions/stats", async ({ query, widget, set }) => {
    requirePermission(widget, "actions.read", set);
    const q = query as any;
    const where: any = { deletedAt: null, isDraft: false };
    if (q.workspace_slug) {
  ...
  .get("/stats/overview", async ({ query, widget, set }) => {
  ...
  .get("/stats/entity/:entity_id", async ({ params: { entity_id }, widget, set }) => {
    requirePermission(widget, "stats.read", set);
  ...
  .get("/stats/period", async ({ query, widget, set }) => {
    requirePermission(widget, "stats.read", set);
    const q = query as any;
  ...
  .get("/users", async ({ query, widget, set }) => {
    requirePermission(widget, "users.read", set);
    const q = query as any;
  ...
  .get("/users/:id", async ({ params: { id }, widget, set }) => {
    requirePermission(widget, "users.read", set);
    const u = await prisma.user.findUnique({
      where: { id },
      select: { id: true, email: true, displayName: true, avatarUrl: true },
  ...
  .get("/entities", async ({ query, widget, set }) => {
  ...
  .get("/entities/:id", async ({ params: { id }, widget, set }) => {
    requirePermission(widget, "entities.read", set);
