...
// local imports
import { buildRecadoPayload, getFieldErrors, toRecadoForm, type TRecadoForm } from "./helpers";
...
function ErroDoCampo({ mensagem }: { mensagem?: string }) {
  ...
  return <p className="text-danger-text mt-1 text-11">{mensagem}</p>;
}
      ...
      <Dialog.Panel width={EDialogWidth.XXXL}>
        <div className="max-h-[90vh] space-y-4 overflow-y-auto p-6">
          ...
          <div>
            ...
            <div className="min-h-[160px] rounded border border-subtle bg-surface-1 py-2">
              {open && workspaceId && (
                <RichTextEditor
                  key={recado?.id ?? "novo"}
                  editable
                  id={`mural-form-${recado?.id ?? "novo"}`}
                  initialValue={form.description_html || "<p></p>"}
          ...
          <div className="flex flex-wrap items-center gap-6">
            <label className="flex items-center gap-2 text-13 text-primary">
              <input
                ...
                onChange={(e) => setCampo("is_pinned", e.target.checked)}
              />
              Fixar no topo
            </label>
            <label className="flex items-center gap-2 text-13 text-primary">
