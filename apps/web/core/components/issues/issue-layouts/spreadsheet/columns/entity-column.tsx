...
export const SpreadsheetEntityColumn = observer(function SpreadsheetEntityColumn({issue, onChange, disabled, onClose}: Props) {
  ...
  const selectedEntity = entities.find((e) => e.id === entityId) ?? issueEntity ?? null;

  useEffect(() => {
    if (!entityId || entities.some((entity) => entity.id === entityId) || !workspaceSlug) return;
    setLoading(true);
    entityService
      .list(workspaceSlug.toString())
      .then(setEntities)
      .finally(() => setLoading(false));
  }, [entityId, entities, workspaceSlug]);
  ...
  return (
    <div
      ref={dropdownRef}
      className="relative h-11 border-b-[0.5px] border-subtle"
    >
      <button
        type="button"
          ...
          <>
            ...
            {!disabled && (
              <span
                ...
                className="shrink-0 rounded hover:bg-layer-2 p-px"
              >
              ...
              filtered.map((e) => (
                <button
                  ...
                  className={cn(
                    ...
                    e.id === entityId && "bg-layer-2 font-medium",
                  )}
                >
                  <Building2 className="h-3.5 w-3.5 shrink-0 text-secondary" />
                  <div className="min-w-0">
                    ...
                    {e.city && (
                      <p className="truncate text-xs text-secondary">
                        {[entityTypeLabel(e.entity_type), e.city].filter(Boolean).join(" · ")}
                      </p>
                    )}
                  </div>
                </button>
