...
import { Plus, Calendar, Building2, User, ChevronRight } from "lucide-react";
...
function CreateVisitModal({ onClose, onCreate }: { onClose: () => void; onCreate: (v: any) => void }) {
  ...
  const [form, setForm] = useState({ entity_id: "", city: "", scheduled_date: "", contacts: "" });
      ...
      <div className="w-full max-w-md rounded-lg bg-surface-1 p-6 shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" onClick={onClose} className="rounded px-3 py-1.5 text-13 text-secondary hover:text-primary">
              Cancelar
            </button>
            <button
              type="submit"
...
function TechnicalVisitsPage() {
  ...
  useEffect(() => { load(); }, [workspaceSlug, statusFilter]);
    ...
    <div className="flex h-full w-full flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between border-b border-subtle px-6 py-4">
        <div>
          ...
          <p className="text-13 text-secondary">{visits.length} visita{visits.length !== 1 ? "s" : ""}</p>
        </div>
        <button
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {!loading && visits.map((visit) => (
          <div key={visit.id} className="flex items-center justify-between border-b border-subtle px-6 py-4 hover:bg-surface-2">
            <div className="flex items-start gap-4">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {visit.visit_number && (
                    <span className="text-11 text-tertiary">#{visit.visit_number}</span>
                  )}
                </div>
                <div className="mt-1 flex items-center gap-3 text-13">
                  ...
                  {visit.city && (
                    <span className="text-secondary">{visit.city}</span>
                  )}
                ...
                {visit.contacts && (
                  <p className="mt-0.5 text-12 text-tertiary">{visit.contacts}</p>
                )}
              </div>
            </div>
            <div className="flex items-center gap-2">
