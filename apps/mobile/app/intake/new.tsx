...
import { Entity, endpoints, Priority } from "@/api";
...
export default function NewIntakeScreen() {
  ...
  const [sheet, setSheet] = useState<null | "priority" | "entity">(null);
  const [saving, setSaving] = useState(false);

  const submit = async () => {
    if (!slug || !projectId || !name.trim()) return;
    const body = { name: name.trim(), description_html: descriptionHtml, priority, entity_id: entityId };
    setSaving(true);
    try {
  ...
  return (
    <Screen scroll>
      ...
      <Row gap={spacing.md}>
        ...
        <Pressable onPress={() => setSheet("entity")} style={{ flex: 1, borderWidth: 1, borderColor: colors.border, borderRadius: 10, padding: spacing.md }}>
          <Text variant="caption">Entidade</Text>
          <Text weight="medium" numberOfLines={1}>{selectedEntity?.name ?? "—"}</Text>
        </Pressable>
      </Row>
