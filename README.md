...
| **Home dashboard** | Static home | **Personal dashboard** in two columns: "Meus chamados" (opened x closed per day, line chart, week / month / quarter), **Tarefas** (the user's open work items with a due date grouped by Hoje / Amanhã / Esta semana / Depois / Atrasados, complete with a checkbox), the **Mural**, an **activity feed** (opened, moved, completed, commented, request answered) and, on the side, a profile card with the month's numbers (closed, open, average resolution time, closing ranking), work items by system, details and last access. Data from `GET /api/workspaces/:slug/home/{serie-de-chamados,tarefas,metricas-do-mes,chamados-por-sistema,perfil,atividade}/`, all scoped to the signed-in user (assignee or creator). Chart colors are theme tokens (`--chart-series-*`). Marketplace widgets still render below it. |
...
| **Developer docs** | — | In-app **Widgets & Custom Integrations** docs page (`/<workspace>/developers/widgets`), linked from the sidebar menu. |
| **Chat mass messaging** | — | **Bulk WhatsApp messages from the chat** (`/<workspace>/chat/disparo`, `chat.disparo` permission): messages with optional image or PDF, recipients filtered by entity type, entity and system with a preview count, a persisted per-phone queue (no duplicate numbers, configurable rate, resumes after restart), per-run log by phone, WhatsApp Status image, Z-API outbound queue view and audit trail. See `.claude/chat-disparo.md`. |
...
| **After-service (pós-atendimento)** | — | The team calls the client after an issue or a technical visit is closed and records **expectation met** (4 levels), **rating** (great/good/bad), **problem solved** (visits only), contact channel and a note; Quality then **verifies** it with a comment. Queue of closed issues and visits with the three legacy tabs (pending after-service, pending verification, verified) and filters by system, entity, period, owner and origin; **"Close and do after-service"** on the issue and the visit; a panel in both detail pages; a **satisfaction report** (distribution by system, entity and period, list by rating, print) gated by `report.view`. Actions `posatendimento.record` (Atendimento, Qualidade, Gestor, admin) and `posatendimento.verify` (Qualidade, Gestor, admin). Legacy data comes in with `scripts/import-pos-atendimento.ts` (MySQL, or the old `pos-N` comments when MySQL is unavailable). This is the **team's** evaluation; the client portal rating is the client's own. See [`.claude/pos-atendimento.md`](./.claude/pos-atendimento.md). |
...
| **Ombudsman, internal reports & résumés** | — | **WhatsApp bot "action" step** (generic flow node that collects fields and calls a registered destination) with three destinations: **ombudsman** (suggestion or complaint; the CNPJ identifies the entity), **"work with us"** (name, position and a PDF résumé) and **update the contact e-mail** (audited before/after). Internal screens: ombudsman with type/read filters, "mark as read" (who and when) and an unread counter in the sidebar; **anonymous internal report** form for every member (nothing links an anonymous report to its author: no author column, day-only date, no audit row, no API-key "last used", no nginx access log line); résumés with position filter, read/interviewed marks, download, permanent deletion and a configurable **retention period** purged daily (LGPD); **responsible contacts e-mail list** by entity, entity type and system, opt-in only, optional internal users, copy and CSV export (audited). **Public "Trabalhe conosco" page** (`/trabalhe-conosco?workspace=<slug>`, no login, served by the api-ts like the client portal) with name, e-mail, phone, position, city, optional message, a mandatory PDF résumé and an LGPD consent box; it is switched on per workspace from the résumés screen ("Receber currículos pelo site", with the link to copy) and protected by a per-IP rate limit and a honeypot field. Résumés show where they came from (site or WhatsApp). Actions `ouvidoria.read`, `denuncia.read`, `curriculo.read`, `contato.export` (Gestor and admin). The bot calls the API through `/api/internal/chat/*` with `CHAT_SERVICE_TOKEN`. See [`.claude/ouvidoria-denuncia-curriculos.md`](./.claude/ouvidoria-denuncia-curriculos.md). |

| **TV wall panels** | — | **Five full-screen panels that open without a Plane login** (`/<workspace>/painel/{ti,qualidade,atendimento,mapa,backups}`), driven by per-TV **API keys** (only the SHA-256 hash is stored; the value is shown once) or by the session of **any active workspace member**, whatever their role. IT and Quality boards with configurable columns, the annual ticket number and a pulsing "client stopped" alert with optional sound; the chat panel with the six legacy tabs rotating on their own and an attendants sidebar; a **Mato Grosso do Sul map** on Leaflet over Esri World Topo terrain tiles (colour, rivers, relief), with everything outside the state dimmed by a mask built from the bundled IBGE GeoJSON, one marker per client city (open-ticket counters, late-backup and offline-server markers, neighbouring cities spread apart so the numbers stay readable), falling back to the bundled SVG drawing when the TV has no internet; and a **backups panel** in the shape of the legacy report. Live over SSE with a 60 s safety refresh. Keys, scopes and column mapping live in **Settings > Painéis de TV** (`panel.manage`); shortcuts sit on the Reports page. Backup and server-status sources are injectable and default to empty. See [`.claude/paineis-tv.md`](./.claude/paineis-tv.md). |
